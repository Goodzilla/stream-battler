...
exports.getEnemyAttackRange = exports.getLegendaryDescription = exports.generateRandomItem = exports.calculateCharacterStats = exports.xpToNextLevel = void 0;
...
const xpToNextLevel = (level) => {
    if (level >= 99) {
        let total1To99 = 0;
        for (let l = 1; l < 99; l++) {
            total1To99 += (0, exports.xpToNextLevel)(l);
        }
        return total1To99;
    }
...
const calculateCharacterStats = (charClass, level, talentsList, passivesList, equippedItems) => {
    const classConfig = constants_js_1.CLASSES[charClass] || constants_js_1.CLASSES.WARRIOR;
    ...
    for (const nodeKey of passivesList) {
        const node = constants_js_1.PASSIVE_SKILL_TREE[nodeKey];
        if (!node)
            continue;
        if (node.stats.maxHp)
    ...
    let atkMult = 1.0;
    let defMult = 1.0;
    const classTalents = constants_js_1.TALENTS[charClass] || {};
...
const generateRandomItem = (itemLevel, rarity, slot, charClass) => {
    let name = '';
    if (slot === 'WEAPON') {
...
const getLegendaryDescription = (affixType) => {
    return '';
};
exports.getLegendaryDescription = getLegendaryDescription;
const getEnemyAttackRange = (name, spriteType) => {
...
exports.getEnemyAttackRange = getEnemyAttackRange;
